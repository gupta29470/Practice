/*
The idea is to first construct the root node of the full binary tree using first 
key in the preorder sequence. Then using given boolean array, we find if the root 
node is an internal node or a leaf node. If root node is an internal node, 
we recursively construct its left and right subtrees.
*/
