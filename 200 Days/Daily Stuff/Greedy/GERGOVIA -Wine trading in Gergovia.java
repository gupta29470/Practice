...
distance between adjacent houses. Transporting one bottle of wine from one house to an 
...
If ai ≥ 0, it means that the inhabitant living in the ith house wants to buy ai bottles of 
wine. If ai < 0, he wants to sell -ai bottles of wine.
...
Output
For each test case print the minimum amount of work units needed so that every inhabitant 
has his demand fulfilled.
...
public class prat {
    ...
    public static void main(String[] args) {
        int numberOfInhabitants = 6;
        int buySell[] = {-1000, -1000, -1000, 1000, 1000, 1000};
        
        System.out.println(minWork(numberOfInhabitants, buySell));
    }
    
    public static int minWork(int numberOfInhabitants, int buySell[]){
        int sum = 0, minimumWork = 0;
        
        for (int index = 0; index < numberOfInhabitants; index++){
            sum+= buySell[index];
            minimumWork+= Math.abs(sum);
        }
        return minimumWork;
    }
